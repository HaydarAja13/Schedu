    ...
    <x-slot name="title">
        <div class="flex justify-between">
            <div class="flex items-center gap-3 mb-8">
                ...
                <h1 class="text-2xl font-bold text-gray-800 font-righteous">Update Data Kelas</h1>
            </div>
        </div>

        <div class="bg-white px-8 pt-8 pb-6 rounded-xl shadow-lg">
            <form action="{{ route('kelas.update', $id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="space-y-6 w-full">
                    <div>
                        ...
                        <input type="text" id="nama_kelas_input" name="nama_kelas" placeholder="Contoh: IF-1A" class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-400" value="{{ old('nama_kelas', $kelas->nama_kelas) }}" required maxlength="20">
                    </div>
                </div>
