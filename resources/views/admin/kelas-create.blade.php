<x-template :role="'admin'" :title="'Tambah Kelas Baru'">
    <x-slot:content>
        <p class="my-2">Tambah kelas baru</p>
        <div class="bg-white rounded-xl h-auto mb-4 p-8">
            <form action="{{ route('kelas.store') }}" method="POST">
                ...
                <div>
                    ...
                    <input type="text" id="nama_kelas" name="nama_kelas" placeholder="cth: kelas A" maxlength="50" class="w-full border border-gray-300 rounded-md px-4 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-400" required>
                </div>

                {{-- Menampilkan error validasi jika ada --}}
                @if ($errors->any())
                    <div class="alert alert-danger text-red-700 mt-3 text-xs font-light">
                        <ul>
