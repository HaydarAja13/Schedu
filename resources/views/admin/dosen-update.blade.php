<x-template :role="'admin'" title="Edit Dosen">
    <x-slot:content>
        <div class="overflow-scroll h-[calc(100vh-200px)]">
            <form action="{{ route('dosen.update' , $id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="mt-4 bg-white rounded-lg p-8">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-x-2">
                        ...
                        <div class="col-span-2 pt-8 gap-y-4 grid h-fit">
                            <div class="flex flex-col md:flex-row md:items-center gap-4">
                                <div class="w-72">
                                    <label class=" whitespace-nowrap  text-sm md:text-base">Nama Dosen <span
                                            class="text-red-500">*</span></label>
                                    <p class="text-xs text-gray-400">Masukan Nama Lengkap Dosen</p>
                                </div>
                                <input type="text" name="nama_dosen" required value="{{ $dosen->nama_dosen }}"
                            ...
                            <div class="flex flex-col md:flex-row md:items-center gap-4">
                                <div class="w-72">
                                    <label class="whitespace-nowrap  text-sm md:text-base">NIP <span
                                            class="text-red-500">*</span></label>
                    ...
                    <div class="grid grid-cols-1 gap-y-4 mt-4 md:mt-8">
                        <div class="flex flex-col md:flex-row md:items-center gap-4">
                            ...
                            <input type="email" name="email" required value="{{ $dosen->email }}"
                                class="w-full rounded-md  px-3 py-1.5 text-sm md:text-sm text-gray-700 outline-1 -outline-offset-1 outline-gray-300 bg-gray-50 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-purple-500" />
                        </div>
                        ...
                        <div class="flex flex-col md:flex-row md:items-center gap-4">
                            ...
                            <input type="number" name="no_hp" required value="{{ $dosen->no_hp }}"
                                class="w-full rounded-md  px-3 py-1.5 text-sm md:text-sm text-gray-700 outline-1 -outline-offset-1 outline-gray-300 bg-gray-50 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-purple-500" />
                        </div>
                        <x-form.button-group page="dosen" />
                    </div>
                </div>
            </form>
        </div>
