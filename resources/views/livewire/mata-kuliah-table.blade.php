                ...
                <table class="w-full table-auto divide-y-2 divide-gray-200" @click.away="selectedRow = null">
                    <thead
                        ...
                        <tr class="*:font-medium *:text-gray-900">
                            ...
                            <th class="whitespace-nowrap px-3 py-2 hover:cursor-pointer" wire:click="setSortBy('sks')">
                                SKS
                            </th>
                            <th class="whitespace-nowrap px-3 py-2 hover:cursor-pointer" wire:click="setSortBy('jam')">
                                Jam
                            </th>
                            <th class="whitespace-nowrap px-3 py-2 hover:cursor-pointer"
                                :class="{ 'hidden': selectedRow }" wire:click="setSortBy('semester')">
                                Semester
                            </th>
                            <th class="whitespace-nowrap px-3 py-2 hover:cursor-pointer"
                                :class="{ 'hidden': selectedRow }" wire:click="setSortBy('jenis')">
                                Jenis
                            </th>
                        </tr>
                    </thead>
                ...
                <ul class="flex items-center justify-end gap-2 text-gray-900">
                    <li>
                        <button type="button"
                            ...
                            aria-label="Previous page" wire:click="previousPage" @if ($mataKuliah->onFirstPage())
                            disabled @endif>
                    ...
                    @php
                    $currentPage = $mataKuliah->currentPage();
                    $lastPage = $mataKuliah->lastPage();
                        ...
                        @for ($page = $start; $page <= $end; $page++) @if ($page==$currentPage) <li
                            ...
                            @if ($end < $lastPage) @if ($end < $lastPage - 1) <li class="flex items-center px-2">...
                                ...
                                <li>
                                    <button type="button"
                                        class="grid size-6 place-content-center rounded text-gray-600 transition-all duration-300 hover:scale-125 hover:text-purple-600 md:size-8 rtl:rotate-180"
                                        aria-label="Next page" wire:click="nextPage" @if ($mataKuliah->currentPage()
                                        == $mataKuliah->lastPage()) disabled @endif>
        ...
        <div class="bg-white absolute inset-0 m-auto h-fit max-w-xs z-50 md:static md:size-full rounded-xl transition-all duration-500 border-2 drop-shadow-lg drop-shadow-[#6B56F6] border-[#6B56F6]"
            ...
            <div class="size-full flex flex-col justify-start p-4">
                <p class="mb-2 text-center text-sm font-semibold">
                    {{ $selectedMataKuliah?->nama_matkul ?? '-' }}
                </p>
                <span
                    class="mx-auto w-fit rounded-full border border-[#6B56F6] bg-[#6B56F6]/25 px-2.5 py-0.5 text-xs font-medium text-[#6B56F6]">Mata
                    Kuliah</span>
                <div class="mt-2 grid gap-y-2">
                    <div>
                        <p class="text-sm font-semibold">Kode Matkul</p>
                        <p class="text-sm">{{ $selectedMataKuliah?->kode_matkul ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-semibold">SKS</p>
                        <p class="text-sm">{{ $selectedMataKuliah?->sks ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-semibold">Jam</p>
                        <p class="text-sm">{{ $selectedMataKuliah?->jam ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-semibold">Semester</p>
                        <p class="text-sm">{{ $selectedMataKuliah?->semester ?? '-' }}</p>
                    </div>
                    <div>
