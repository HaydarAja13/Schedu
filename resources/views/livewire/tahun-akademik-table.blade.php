<div x-data="{ selectedRow: null, showAlert: false }">
    <x-table-properties :subtitle="'Menampilkan Data Tahun Akademik'" link="tahun-akademik-create"></x-table-properties>
        ...
        <div :class="selectedRow ? 'overflow-x-hidden overflow-y-auto bg-white rounded-xl md:col-span-3' :
                ...
                <table class="w-full table-auto divide-y-2 divide-gray-200" @click.away="selectedRow = null">
                    <thead
                        ...
                        <tr class="*:font-medium *:text-gray-900">
                            ...
                            <th class="whitespace-nowrap px-3 py-2 hover:cursor-pointer"
                                wire:click="setSortBy('tahun_ajaran')">
                                Tahun Ajaran</th>
                            <th class="whitespace-nowrap px-3 py-2 hover:cursor-pointer"
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr :class="[
                            ...
                            ]" @click="selectedRow = '{{ $data->id }}'; $wire.selectTahunAkademik({{ $data->id }})">
            ...
            <div
                ...
                <div class="mt-2 text-sm font-light text-gray-800">
                    Menampilkan
                    {{ $tahunAkademik->firstItem() ?? 0 }}
                    hingga
                    {{ $tahunAkademik->lastItem() ?? 0 }}
                    dari
                    {{ $tahunAkademik->total() }}
                </div>
                <ul class="flex items-center justify-end gap-2 text-gray-900">
                    <li>
                        <button type="button"
                            ...
                            aria-label="Previous page" wire:click="previousPage" @if ($tahunAkademik->onFirstPage())
                            disabled @endif>
                    ...
                    @php
                    $currentPage = $tahunAkademik->currentPage();
                    $lastPage = $tahunAkademik->lastPage();
                        ...
                        @for ($page = $start; $page <= $end; $page++) @if ($page==$currentPage) <li
                            ...
                            @if ($end < $lastPage) @if ($end < $lastPage - 1) <li class="flex items-center px-2">...
                                ...
                                <li>
                                    <button type="button"
                                        class="grid size-6 place-content-center rounded text-gray-600 transition-all duration-300 hover:scale-125 hover:text-purple-600 md:size-8 rtl:rotate-180"
                                        aria-label="Next page" wire:click="nextPage" @if ($tahunAkademik->currentPage()
                                        == $tahunAkademik->lastPage()) disabled @endif>
        ...
        <div x-cloak
            ...
            <div class="size-full flex flex-col justify-start p-4">
                <p class="mb-2 text-center text-sm font-semibold">
                    {{ $selectedTahunAkademik?->tahun_ajaran ?? '-' }}
                </p>
                <span
                    class="mx-auto w-fit rounded-full border border-[#6B56F6] bg-[#6B56F6]/25 px-2.5 py-0.5 text-xs font-medium text-[#6B56F6]">Tahun
                    Akademik</span>
                <div class="mt-2 grid gap-y-2">
                    <div>
                        <p class="text-sm font-semibold">Nama Tahun Ajaran</p>
                        <p class="text-sm">{{ $selectedTahunAkademik?->tahun_ajaran ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-semibold">Status</p>
                        <p class="text-sm">
                            {{ $selectedTahunAkademik?->status === '0' ? 'Tidak Aktif' :
                            ($selectedTahunAkademik?->status ===
                            '1' ? 'Aktif' : '-') }} </p>
                    </div>
                </div>
                <div class="mt-8 md:mt-auto flex flex-col gap-2">
                    <a class="inline-block rounded-xl bg-gradient-to-tr from-[#8C4AF2] to-[#6B56F6] px-12 py-2 text-center text-sm font-medium text-white"
                        href="{{ route('admin.tahun-akademik-update', $selectedTahunAkademik?->id ?? '') }}">
                        Ubah Data
                    </a>
                    <button
    ...
    <div x-cloak x-show="showAlert" class="mb-4">
        <x-alert titleModal="Peringatan" contentModal="Apakah anda yakin ingin menghapus data ini?" type="warning"
            :route="route('tahun-akademik.destroy', $selectedTahunAkademik?->id ?? '')" />
    </div>
</div>