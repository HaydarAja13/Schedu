    ...
    <div class="grid grid-cols-4 items-center justify-start gap-x-6 size-full">
            ...
            <table class="min-w-full divide-y-2 divide-gray-200" @click.away="selectedRow = null">
                <thead
                    ...
                    <tr class="*:font-medium *:text-gray-900">
                        ...
                        <th class="px-3 py-2 whitespace-nowrap hover:cursor-pointer" wire:click="setSortBy('kode_matkul')">
                            Kode Mata Kuliah
                        </th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr :class="[
                        ...
                        <td class="px-3 py-3 whitespace-nowrap text-sm">{{ $data->kode_matkul }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
                ...
                <ul class="flex justify-end gap-1 text-gray-900">
                    <li>
                        <button type="button"
                            ...
                            aria-label="Previous page" wire:click="previousPage" @if ($matkul->onFirstPage())
                            disabled
                            @endif>
                    ...
                    @php
                    $currentPage = $matkul->currentPage();
                    $lastPage = $matkul->lastPage();
                        ...
                        @for ($page = $start; $page <= $end; $page++) @if ($page==$currentPage) <li
                            ...
                            @if ($end < $lastPage) @if ($end < $lastPage - 1) <li class="flex items-center px-2">...
                                ...
                                <li>
                                    <button type="button"
                                        class="grid size-8 place-content-center rounded text-gray-600 hover:text-purple-600 hover:scale-125 transition-all duration-300 rtl:rotate-180"
                                        aria-label="Next page" wire:click="nextPage" @if ($matkul->currentPage() ==
                                        $matkul->lastPage()) disabled @endif>
        ...
        <div class="bg-white size-full rounded-xl transition-all duration-500 border-2 drop-shadow-lg drop-shadow-[#6B56F6] border-[#6B56F6]"
            ...
            <div class="size-full flex flex-col justify-start p-4 h-full">
                ...
                <span
                    class="bg-[#6B56F6]/25 w-fit mx-auto text-[#6B56F6] text-xs font-medium px-2.5 py-0.5 rounded-full border border-[#6B56F6]">Mata Kuliah</span>
                <div class="grid gap-y-2 mt-2 flex-grow">
                    <div>
                        <p class="font-semibold text-sm">Kode Mata Kuliah</p>
                        <p class="text-sm">{{ \Illuminate\Support\Str::limit($selectedMataKuliah?->kode_matkul ?? '-', 30) }}</p>
                    </div>
                    <div>
                        <p class="font-semibold text-sm">Nama</p>
                        <p class="text-sm">{{ \Illuminate\Support\Str::limit($selectedDosen?->nama_matkul ?? '-', 30) }}</p>
                    </div>
                </div>
