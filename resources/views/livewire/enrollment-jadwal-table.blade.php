<div x-data="{ selectedRow: null, showAlert: false }">
    <x-table-properties :subtitle="'Menampilkan Data Enrollment Jadwal'" link="enrollment-jadwal-create">
    </x-table-properties>
        ...
        <div :class="selectedRow ? 'overflow-x-hidden overflow-y-auto bg-white rounded-xl md:col-span-3' :
            ...
            <div class="w-full overflow-x-scroll md:overflow-x-hidden">
                <table @click.away="selectedRow = null" class="w-full table-auto divide-y-2 divide-gray-200">
                    <thead
                        ...
                        <tr class="*:font-medium *:text-gray-900">
                            ...
                            <th class="whitespace-nowrap px-3 py-2 hover:cursor-pointer"
                                wire:click="setSortBy('id_mata_kuliah')">
                                Mata Kuliah</th>
                            <th class="whitespace-nowrap px-3 py-2 hover:cursor-pointer"
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        $no = 1;
                        @endphp
                        @foreach ($enrollmentJadwal as $data)
                        <tr :class="[
                            ...
                            ]" @click="selectedRow = '{{ $data->id }}'; $wire.selectEnrollmentJadwal({{ $data->id }})">
            ...
            <div
                ...
                <div class="mt-2 text-sm font-light text-gray-800">
                    Menampilkan
                    {{ $enrollmentJadwal->firstItem() ?? 0 }}
                    hingga
                    {{ $enrollmentJadwal->lastItem() ?? 0 }}
                    dari
                    {{ $enrollmentJadwal->total() }}
                </div>
                <ul class="flex items-center justify-end gap-2 text-gray-900">
                    <li>
                        <button @if ($enrollmentJadwal->onFirstPage()) disabled @endif
                    ...
                    @php
                    $currentPage = $enrollmentJadwal->currentPage();
                    $lastPage = $enrollmentJadwal->lastPage();
                    ...
                    if ($currentPage <= 3) { $start=1; $end=min(3, $lastPage); } elseif ($currentPage>= $lastPage - 2) {
                        ...
                        @for ($page = $start; $page <= $end; $page++) @if ($page==$currentPage) <li
                            ...
                            @if ($end < $lastPage) @if ($end < $lastPage - 1) <li class="flex items-center px-2">...
                                ...
                                <li>
                                    <button @if ($enrollmentJadwal->currentPage() == $enrollmentJadwal->lastPage())
                                        disabled @endif
        ...
        <div x-cloak :class="{ 'hidden': !selectedRow }"
            ...
            <div class="flex size-full flex-col justify-start p-4">
                <p class="mb-2 text-center text-sm font-semibold">
                    {{ $selectedEnrollmentJadwal?->mataKuliah->nama_matkul ?? '-' }}
                </p>
                <span
                ...
                <div class="mt-2 grid gap-y-2">
                    <div>
                        ...
                        <p class="text-sm">
                            {{ ($selectedEnrollmentJadwal?->enrollmentKelas->programStudi->kode_prodi ?? '-') .
                            '-' .
                            ($selectedEnrollmentJadwal?->enrollmentKelas->angkatan->tahun_angkatan ?? '-') .
                            ($selectedEnrollmentJadwal?->enrollmentKelas->kelas->nama_kelas ?? '-') }}
                        </p>
                    </div>
                    <div>
                        <p class="text-sm font-semibold">Program Studi</p>
                        <p class="text-sm">
                            {{ $selectedEnrollmentJadwal?->enrollmentKelas->programStudi->nama_prodi ?? '-' }}
                        </p>
                    </div>
                    <div>
                        <p class="text-sm font-semibold">Dosen</p>
                        <p class="text-sm">{{ $selectedEnrollmentJadwal?->dosen->nama_dosen ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-sm font-semibold">Tahun Akademik</p>
                        <p class="text-sm">
                            {{ $selectedEnrollmentJadwal?->enrollmentKelas->tahunAkademik->tahun_ajaran ?? '-' }}
                        </p>
                    </div>
                </div>
                <div class="mt-8 flex flex-col gap-2 md:mt-auto">
                    <a class="inline-block rounded-xl bg-gradient-to-tr from-[#8C4AF2] to-[#6B56F6] px-12 py-2 text-center text-sm font-medium text-white"
                        href="{{ route('admin.enrollment-jadwal-update', $selectedEnrollmentJadwal?->id ?? '') }}">
                        Ubah Data
                    </a>
