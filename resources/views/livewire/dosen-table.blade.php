<div x-data="{ selectedRow: null }">
    {{-- <x-alert></x-alert> --}}
    <x-table-properties :subtitle="'Data Dosen'"></x-table-properties>
                ...
                <thead
                    ...
                    <tr class="*:font-medium *:text-gray-900">
                        ...
                        <th class="px-3 py-2 whitespace-nowrap hover:cursor-pointer"
                            wire:click="setSortBy('nama_dosen')">
                            Nama</th>
                        <th class="px-3 py-2 whitespace-nowrap hover:cursor-pointer" wire:click="setSortBy('nip')">NIP
                        </th>
                    ...
                    @foreach ($dosen as $data)
                        <tr :class="[
                            ...
                            <td class="py-3 whitespace-nowrap text-sm"
                                :class="selectedRow === '{{ $data->id }}' ? 'pl-10' : 'px-3'">{{ $no++ }}
                            </td>
                ...
                <ul class="flex justify-end gap-1 text-gray-900">
                    <li>
                        <button type="button"
                            ...
                            @if ($dosen->onFirstPage()) disabled @endif>
                    ...
                    <li>
                        <button type="button"
                            ...
                            @if ($dosen->currentPage() == $dosen->lastPage()) disabled @endif>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                                stroke="currentColor" class="size-6">
            ...
            <div class="size-full flex flex-col justify-start p-4 h-full">
                ...
                <div class="mt-auto flex flex-col gap-2">
                    <a class="inline-block rounded-xl bg-gradient-to-tr from-[#8C4AF2] to-[#6B56F6] px-12 py-2 text-sm text-center font-medium text-white"
                        href="{{ route('admin.dosenEdit', $selectedDosen?->id ?? 0) }}">
                        Ubah Data
                    </a>
                    <form action="{{ route('admin.dosen.destroy', $selectedDosen?->id ?? 0) }}" method="POST"
                        onsubmit="return confirm('Yakin ingin hapus data?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                            class="inline-block rounded-xl border border-red-600 px-12 py-2 text-sm text-center font-medium text-white bg-red-600">
                            Hapus Data
                        </button>
                    </form>
