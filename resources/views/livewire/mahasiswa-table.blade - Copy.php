<div x-data="{ selectedRow: null }">
    ...
    <div class="grid grid-cols-4 shadow items-center justify-start gap-x-6 size-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr :class="[
                        ...
                        <td class="px-6 py-3 flex items-center gap-3 whitespace-nowrap">
                            @if ($data->foto)
                                <img src="{{ asset($data->foto) }}" alt="Foto Profil" class="w-8 h-8 rounded-full object-cover">
                            @else
                                <div class="w-8 h-8 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 text-white flex items-center justify-center font-semibold text-xs uppercase">
                                    {{ $initials }}
                                </div>
                            @endif
                        ...
                        <td class="px-6 py-3 text-purple-600 underline">{{ $data->email }}</td>
        ...
        <div class="bg-white size-full rounded-xl transition-all duration-500 border-2 border-[#6B56F6]"
            ...
            <div class="size-full flex flex-col justify-start p-4">
                @if ($selectedMahasiswa?->foto)
                    <img src="{{ asset($selectedMahasiswa->foto) }}" alt="Foto Profil" class="rounded-full mb-2 size-32 mx-auto">
                @else
                    @php
                        $initials = collect(explode(' ', $selectedMahasiswa?->nama_mahasiswa ?? ''))
                                        ...
                                        ->implode('');
                    @endphp
                    <div class="rounded-full mb-2 size-32 mx-auto bg-gradient-to-r from-indigo-500 to-purple-500 text-white flex items-center justify-center font-bold text-4xl">
                        {{ $initials }}
                    </div>
                @endif
