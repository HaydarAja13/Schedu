      ...
      <div class="grid grid-cols-1 w-full mt-4">
        ...
        <div class="bg-white px-10 grid gap-y-2 py-4 rounded-b-lg shadow-lg">
          <div class="relative flex items-center justify-center bg-white size-fit p-1.5 -mt-16 z-30 rounded-full">
            {{-- Modified line: Use $user->foto_profil or your actual column name --}}
            <img src="{{ $user->foto_profil ? asset('storage/' . $user->foto_profil) : 'https://placehold.co/100' }}" alt="Profile Picture" class="size-22 md:size-28 rounded-full">
          </div>
          <p class="text-lg md:text-xl font-semibold">{{ $user->nama_mahasiswa }}</p>
