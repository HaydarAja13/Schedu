<x-template :role="'mahasiswa'" title="Halo, {{ $user->nama_mahasiswa ?? '-' }}">
  <x-slot:content>
    <div class="grid grid-cols-1 w-full mt-4">
      <div class="relative h-28 overflow-hidden rounded-t-lg">
        ...
        <div class="relative flex items-end justify-end size-full z-20 p-4">
          <img src="{{ asset('images/schedu-logo-white.svg') }}" alt="Logo Schedu" class="w-20 md:w-26">
        </div>
      </div>
      <div class="bg-white px-10 grid gap-y-2 py-4 rounded-b-lg shadow-lg">
        ...
        <p class="text-lg md:text-xl font-semibold">{{ $user->nama_mahasiswa ?? 'Nama Mahasiswa' }}</p>
        <p class="rounded-full font-semibold uppercase text-sm whitespace-nowrap text-purple-700 w-fit">
          {{ $role ?? 'Mahasiswa' }}</p> {{-- Menambahkan fallback untuk role --}}
        
        <div class="grid grid-cols-1 md:grid-cols-3 gap-y-2 items-center mt-2">
          <p class="font-medium text-sm md:text-base">NIM</p>
          <div class="mt-2 col-span-2">
            {{-- PERBAIKAN: Menambahkan fallback value --}}
            <input type="text" name="nim" id="nim" readonly value="{{ $user->nim ?? '-' }}"
        ...
        <div class="flex justify-end items-center">
          ...
          <a type="button" href="/mahasiswa/profile-update"
